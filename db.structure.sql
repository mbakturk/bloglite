...
CREATE TRIGGER decrease_t_post_count AFTER DELETE ON t_post BEGIN UPDATE t_metadata SET value = value - 1 WHERE "key" = 't_post_count'; END;

-- Trigger: increase_t_post_count
CREATE TRIGGER increase_t_post_count AFTER INSERT ON t_post BEGIN INSERT INTO t_metadata ("key", value) SELECT 't_post_count', '0' WHERE NOT EXISTS(SELECT 1 FROM t_metadata WHERE "key" = 't_post_count'); UPDATE t_metadata SET value = value + 1 WHERE "key" = 't_post_count'; END;
